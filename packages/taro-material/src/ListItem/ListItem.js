...
export default class RMListItem extends Component {
  handleClick = (...args) => {
    const {
      disabled,
    } = this.props
  ...
  render () {
    ...
    return (
      <View className={rootClass} onClick={this.handleClick}>
        ...
        {iconThumb && (
          <View className='at-list__item-thumb item-thumb'>
            <RMIcon className='item-thumb-info' color={iconThumbColor} fontSize='inherit' block={true}>{iconThumb}</RMIcon>
          </View>
        )}
        <View className='at-list__item-content item-content'>
          <View className='item-content__info'>
            <View className='item-content__info-title'><RMTypography className='subheading'>{title}</RMTypography></View>
            {note && <View className='item-content__info-note'><RMTypography className='caption'>{note}</RMTypography></View>}
          </View>
        </View>
        <View className='at-list__item-extra item-extra'>
          {extraText && <View className='item-extra__info'><RMTypography className='body1'>{extraText}</RMTypography></View>}
          ...
          {extraIconThumb && (
            <View className='item-extra__image'>
              <RMIcon
                className='item-extra__image-info' fontSize='inherit' block={true}
              >{extraIconThumb}</RMIcon>
            </View>
          )}
          ...
          {arrowIcon && (
            <View className='item-extra__icon' style={{
              margin: '0px -16px'
            }}>
              <RMButton variant='text' color='default' size='normal' disabled={disabled}>
                <RMIcon color='inherit' fontSize='inherit'>{arrowIcon}</RMIcon>
              </RMButton>
            </View>
          )}
        </View>
