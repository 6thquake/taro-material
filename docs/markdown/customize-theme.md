...
新建一个主题样式文件，例如 `custom-variables.scss`，并写入以下内容：（如有需要，可使用[自定义主题生成器](https://nervjs.github.io/taro-material-theme-preview/)）

:::demo
```scss
/* 改变主题变量，具体变量名可查看 taro-material/dist/style/variables/default.scss 文件 */
$color-brand: #6190E8;

/* 引入 Taro UI 默认样式 */
@import "~taro-material/dist/style/index.scss";
```
:::

> 覆写的变量，需要在引入 taro ui 默认样式之前定义，[默认主题变量命名](https://github.com/NervJS/taro-material/blob/dev/src/style/variables/default.scss)
...
**P.S. `addGlobalClass` 这个 API 只对 Page 上的 class 起作用。换言之，如果在自定义的组件中使用 `taro-material`，是无法在自定义组件内部通过 `全局样式类` 的方式去更改组件样式的。**
